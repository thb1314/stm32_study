...
__IO uint16_t timer = 1;
__IO uint8_t dir = 1;
void TIM_Init(void)	 
{
	//将TiM1_CH1（）设置成推挽复用输出
	if(0 == B_RCC->APB2ENR.IOPAEN)
		B_RCC->APB2ENR.IOPAEN = 1;
	// 
	PA->CP.CONFIG[8].MODE[1] = 1;
	PA->CP.CONFIG[8].MODE[0] = 1;
	PA->CP.CONFIG[8].CNF[1] = 1;
	PA->CP.CONFIG[8].CNF[0] = 0;
	...
	//输出比较模式(PWM2)
	B_TIM1->CCMR1.IC1F_OR_OC1[2] = 1;
	B_TIM1->CCMR1.IC1F_OR_OC1[1] = 1;
	B_TIM1->CCMR1.IC1F_OR_OC1[0] = 1;
	//输出预装载使能
	B_TIM1->CCMR1.IC1PSC_OR_OC1[1] = 1;

	//高级定时器 还需要使能 MOE
	B_TIM1->BDTR.MOE = 1;
	//TIM1 和TIM8 捕获/比较寄存器 1
	TIM1->CCR1 = timer;
	//低电平有效
	B_TIM1->CCER.CC1P = 1;
	//输出使能
...
int main(void)
{
	...
	//PE5 设置成上拉输入
	PE->CP.CONFIG[5].MODE[1] = 0;
	PE->CP.CONFIG[5].MODE[0] = 0;
	PE->CP.CONFIG[5].CNF[1] = 1;
